...
final class SpeechGeneratorViewModel: ObservableObject {
    ...
    @MainActor
    func initialize(downloadProgress: (@Sendable (Progress) -> Void)? = nil)
        async throws
    {
        ...
        do {
            ...
            logger.info(
                "F5TTS model initialized successfully in \(String(format: "%.2f", duration))s"
            )
            isInitialized = true
        } catch {
            logger.error(
                "Failed to initialize F5TTS: \(error.localizedDescription)")
    ...
    func generateSpeechAudio(text: String) async throws -> MLXArray {
        ...
        logger.info(
            "Starting speech generation for text: \(text.prefix(50))...")
        ...
        if let reference = selectedReference {
            logger.info(
                "Using reference audio: \(reference.audio.lastPathComponent)")
        ...
        let duration = CFAbsoluteTimeGetCurrent() - startTime
        logger.info(
            "Speech generation completed in \(String(format: "%.2f", duration))s"
        )
        return result
    }
    ...
    func saveAudio(audio: MLXArray, to outputURL: URL) throws {
        ...
        let duration = CFAbsoluteTimeGetCurrent() - startTime
        logger.info(
            "Audio saved successfully in \(String(format: "%.2f", duration))s")
    }
